...
(() => {
  ...
  function ChatWidget() {
    ...
    const panel = h("div", { style: { position: "fixed", right: 22, bottom: 84, zIndex: 240, width: "min(400px, calc(100vw - 32px))", height: "min(620px, calc(100vh - 130px))", display: "flex", flexDirection: "column", background: "var(--bg-raised)", border: "1px solid var(--border-oracle)", borderRadius: 14, boxShadow: "0 24px 60px rgba(0,0,0,0.6)", overflow: "hidden" } },
      // header
      h("div", { style: { display: "flex", alignItems: "center", gap: 10, padding: "12px 50px 12px 14px", borderBottom: "1px solid var(--border-subtle)", background: "rgba(212,169,78,0.05)" } },
      ...
      h("button", { onClick: (e) => { e.stopPropagation(); setOpen(false); }, "aria-label": "Close", "data-sfx": "", style: { position: "absolute", top: 6, right: 8, zIndex: 4, background: "none", border: "none", color: "var(--text-secondary)", fontSize: 30, lineHeight: 1, cursor: "pointer", padding: "6px 12px", minWidth: 46, minHeight: 46, display: "inline-flex", alignItems: "center", justifyContent: "center" } }, "×"),
      // body
